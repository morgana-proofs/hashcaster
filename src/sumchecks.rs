...
pub trait AdmissibleMatrixArray<const I: usize, const O: usize> {
    fn logsize_in() -> usize;
    fn logsize_out() -> usize;
    fn apply(src: [&[F128]; I], dst: [&mut[MaybeUninit<F128>]; O]);
    fn apply_transposed(src: [&[F128]; O], dst: [&mut[MaybeUninit<F128>]; I]);
}